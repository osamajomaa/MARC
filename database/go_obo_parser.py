...
Version 1.0
Modified by Iddo Friedberg 2013-2014
Source: http://techoverflow.net/blog/2013/11/18/a-geneontology-obo-v1.2-parser-in-python/
...
def processGOTerm(goTerm):
    ...
    for key, value in ret.iteritems():
        if key == 'is_a':
            ...
            for val in value:
                values = {}
                term,comment = val.split("!")
                values['id'] = term.strip()
                values['comment'] = comment.strip()
                processed_val.append(values)
        else:
            processed_val = value

        ret[key] = processed_val
    return ret
